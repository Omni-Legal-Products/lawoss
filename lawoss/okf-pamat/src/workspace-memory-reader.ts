...
export function readWorkspaceMemorySnapshot(directory: string, options: WorkspaceMemoryOptions = {}, ownOperation?: string): WorkspaceMemoryReport {
  ...
  try {
    ...
    const profile: unknown = JSON.parse(profileText.content);
    if (!isObject(profile) || profile.version !== 1 || !safeId(profile.matterId) || !Array.isArray(profile.roots) || !Array.isArray(profile.sources)) throw new Error("Invalid version 1 memory profile.");
    ...
    if (profile.sources.length === 0 || profile.sources.length > WORKSPACE_MEMORY_LIMITS.sources || profile.roots.length === 0 || profile.roots.length > WORKSPACE_MEMORY_LIMITS.sources) throw new Error("Invalid profile source/root count.");
    ...
    for (const root of profile.roots) {
      if (!isObject(root) || !safeId(root.id) || roots.has(root.id) || typeof root.path !== "string" || root.path.length === 0 || root.path.includes("\0") || root.path.includes("\\") || root.path.split("/").includes("..")) throw new Error("Invalid or duplicate root.");
