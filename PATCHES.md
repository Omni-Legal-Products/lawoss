...
| `apps/app/src/react-app/domains/session/chat/permission-approval-modal.tsx` | V `PermissionApprovalPanel` aj `PermissionApprovalModal` nad detailom príkazu `{lite && memoryWriteProposal ? <MemoryWriteNotice proposal={memoryWriteProposal} /> : null}`, kde `memoryWriteProposal = describeMemoryWrite(String(metadata.command ?? ""))` (+3 importy, +2× 1 riadok) | Lite karta pri žiadosti o spustenie `okf-memory write …`: záznam, dôvod, náhľad/zápis po schválení, lhôty vždy potvrdzuje advokát; v pro sa nezobrazí (SSR snapshot `useUiMode()` je vždy „pro“), tlačidlá a zvyšok panela beze zmeny | VŘ | feat/lawoss-lite |
