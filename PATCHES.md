...
| `apps/desktop/package.json` | +2 súbory v skripte `test` (`electron/artifact-name.test.mjs`, `electron/window-allowlist.test.mjs`) | Aby sa názov artefaktov nemohol ticho vrátiť; aby allowlist okna bežal v CI (skript vymenúva testy explicitne) | MČ, VŘ | plan/branding-pass-a-alfa; fix/okno-len-vlastna-adresa |
