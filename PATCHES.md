...
| `apps/app/src/react-app/domains/settings/shell/settings-page.tsx` | `getWorkspaceSettingsTabs()` a `getGlobalSettingsTabs()` prejdú `liteSettingsTabs(…, currentUiMode())` (za `hideCommercialTabs`); prázdna skupina Workspace v bočnom paneli `hidden` (+2 importy, 3 riadky) | Lite nastavenia: prehľad, AI, Personalizácia (kancelária), Vzhľad; v pro `liteSettingsTabs` vracia to isté pole | VŘ | feat/lawoss-lite |
| `apps/app/src/react-app/domains/settings/shell/settings-shell.tsx` | `useUiMode()` v `SettingsShell` (prekreslenie zoznamov záložiek) + `<UiModeSwitch />` vedľa `LanguageSwitcher` v oboch hlavičkách (+2 importy, +3 riadky) | Prepnutie Jednoduchý/Pokročilý na jednom mieste, bez reštartu | VŘ | feat/lawoss-lite |
