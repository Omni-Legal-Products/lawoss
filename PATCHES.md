...
| `apps/desktop/package.json` | +2 súbory v skripte `test` (`electron/artifact-name.test.mjs`, `scripts/dev-server-identity.test.mjs`) | Aby sa názov artefaktov nemohol ticho vrátiť; aby identifikácia dev servera bežala v CI (skript vymenúva testy explicitne) | MČ, VŘ | plan/branding-pass-a-alfa; fix/electron-dev-vlastny-vite |
