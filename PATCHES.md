...
| `apps/app/src/react-app/domains/session/chat/permission-approval-modal.tsx` | V `PermissionApprovalPanel` aj `PermissionApprovalModal` nad detailom príkazu `{lite && memoryWriteProposal ? <MemoryWriteNotice proposal={memoryWriteProposal} /> : null}`, kde `memoryWriteProposal = describeMemoryWrite(String(metadata.command ?? ""))` (+3 importy, +2× 1 riadok) | Lite karta pri žiadosti o spustenie `okf-memory write …`: vec, záznam, dôvod, schvaľovateľ, náhľad/zápis po schválení, lehoty vždy potvrdzuje advokát; pro nie je dotknuté vďaka podmienke `lite &&` (v pro sa karta nikdy nevykreslí), tlačidlá a zvyšok panela beze zmeny | VŘ | feat/lawoss-lite |
