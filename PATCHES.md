...
| `apps/app/src/react-app/domains/settings/pages/general-view.tsx` | Riadky prehľadu filtrované `isSettingsTabVisible(tab, currentUiMode())`; prázdna skupina sa nevykreslí; `useUiMode()` kvôli prekresleniu (+2 importy, +4 riadky) | Prehľad nesmie v lite odkazovať na skryté záložky (rovnaký dôvod ako filter `HIDDEN_SETTINGS_TABS`) | VŘ | feat/lawoss-lite |
