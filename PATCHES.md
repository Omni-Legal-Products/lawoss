...
|---|---|---|---|---|
| `scripts/i18n-audit.mjs` | `resolveSpreadImports()`: pred eval rozvinie `...názov` spready v locale súboroch načítaním `export const názov = {…}` z importovaných doménových slovníkov (`apps/app/src/lawoss/i18n/*`) | Od #87 locale súbory skladajú LAWOSS slovníky spreadom; naivný eval padal na `setupEn is not defined` a `ci-i18n` bol na `dev` červený | MČ | fix/i18n-audit-spready |
