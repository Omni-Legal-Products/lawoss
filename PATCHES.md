...
| `apps/desktop/electron-builder.yml` | `productName` → `LAWOSS`; publisher → `Omni-Legal-Products/lawoss`; icon files in `resources/icons/**` replaced by LAWOSS badge (binary); public protocol label, macOS permission descriptions, and installer `artifactName` use LAWOSS | LAWOSS branding and fork-owned updater metadata; `appId` and `legalwork://` scheme intentionally unchanged (keychain/user-data/update/deep-link continuity — separate ADR if ever) | MČ/MF | fix/updater-release-assets; codex/lawoss-branding-phase1 |
| `apps/desktop/package.json` | Public description and author changed to LAWOSS/Omni-Legal-Products; internal package scope remains `@legalwork/desktop` | Remove upstream publisher metadata without changing workspace resolution | MF | codex/lawoss-branding-phase1 |
| `.github/workflows/release-macos-aarch64.yml`, `.github/workflows/alpha-macos-aarch64.yml`, `.github/workflows/alpha-windows-x64.yml` | Stable/alpha release labels and download URLs use LAWOSS; installer globs and alpha manifest rewrites use `lawoss-*`; stable direct links target the fork | Ensure GitHub builds and updater pointers publish the LAWOSS distribution, not upstream | MF | codex/lawoss-branding-phase1 |
| `.github/workflows/ci-tests.yml`, `.github/workflows/telegram-notify.yml` | CI workflow display name `LegalWork Tests` → `LAWOSS Tests` and Telegram workflow subscription aligned | Keep GitHub and Telegram event labels consistent with the fork product | MF | codex/lawoss-branding-phase1 |
| `.github/workflows/download-stats.yml`, `scripts/release/report-download-stats.mjs` | Download/traffic snapshots target `Omni-Legal-Products/lawoss`; LAWOSS variables are preferred while legacy variable names remain accepted | Keep release analytics attached to the fork and preserve existing repository configuration | MF | codex/lawoss-branding-phase1 |
| `scripts/release/apply-signpath-windows-artifact.mjs` | Signed installer discovery uses the public `lawoss-win-x64-*` artifact prefix | Keep Windows signing compatible with LAWOSS installer filenames | MF | codex/lawoss-branding-phase1 |
| `apps/app/src/app/lib/electron-alpha.ts`, `apps/app/src/app/lib/release-channels.ts`, `apps/app/src/react-app/shell/loading-overlay.tsx`, `apps/app/src/react-app/design-system/web-unavailable-surface.tsx` | Public update, release, and desktop-download URLs target the LAWOSS fork; Web unavailable copy uses LAWOSS branding | Prevent application UI and updater channels from sending users to upstream LegalWork | MF | codex/lawoss-branding-phase1 |
| `apps/desktop/electron/main.mjs`, `apps/desktop/electron/updater.mjs` | Electron display name, release page, and documented fallback URL use LAWOSS | Keep packaged app and release links aligned with the fork | MF | codex/lawoss-branding-phase1 |
| `apps/orchestrator/scripts/postinstall.mjs`, `apps/orchestrator/src/cli.ts`, `apps/orchestrator/README.md`, `apps/orchestrator/package.json` | Orchestrator release URLs and public package/documentation metadata target LAWOSS | Ensure sidecar installation and package discovery use the fork | MF | codex/lawoss-branding-phase1 |
| `apps/opencode-router/install.sh`, `apps/opencode-router/README.md`, `apps/opencode-router/package.json`, `apps/server/package.json` | Installer, documentation, and package repository metadata target the LAWOSS fork | Keep auxiliary distribution entry points on the fork | MF | codex/lawoss-branding-phase1 |
| `.github/ISSUE_TEMPLATE/bug.yml` | Issue form description and version labels use LAWOSS | Keep GitHub support entry points consistent with the product | MF | codex/lawoss-branding-phase1 |
