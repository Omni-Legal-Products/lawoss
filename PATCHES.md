...
|---|---|---|---|---|
| `.github/workflows/alpha-macos-aarch64.yml` | Z `env` kroku „Package Electron alpha (macOS, signed + notarized)“ odstránený riadok `MACOS_NOTARIZE: true` (−1 riadok); hodnotu dedí z `env` jobu | `env` kroku sa započítava do jeho vlastného `if:`, takže `env.MACOS_NOTARIZE == 'true'` platilo vždy a nepodpísaná vetva sa nikdy nespustila; run 35769444113 s `vars.MACOS_NOTARIZE=false` padol na podpisovaní bez certifikátu. `release-macos-aarch64.yml` túto chybu nemá | MČ | fix/alpha-macos-unsigned |
| `.github/workflows/alpha-windows-x64.yml`, `release-macos-aarch64.yml` | Maska Windows inštalátora `legalwork-win-x64-*.exe` → `lawoss-win-x64-*.exe` (2 riadky) | `a2facdf1` premenoval `artifactName` na `lawoss-…`, masky ostali staré; alfa run 35752713989 padol na „found 0“ hoci build prešiel | MČ | fix/windows-installer-maska |
| `scripts/release/prepare.mjs` | Verzia `x.y.z-lawoss.N` (AGENTS.md): `--set` vyžaduje tento tvar, bez `--set` sa zvýši `N` posledného `v*-lawoss.*` tagu; fetch ťahá len LAWOSS tagy cez `ls-remote` namiesto `--tags`; odstránené `patch/minor/major` | Holé `vX.Y.Z` koliduje s upstream tagom rovnakého mena (lokálne `v0.1.14` ≠ origin `v0.1.14`), `--tags` preto padal na clobber a auto-bump by nasledoval číslovanie LegalWorku | MČ | fix/release-lawoss-suffix |
| `scripts/i18n-audit.mjs` | `resolveSpreadImports()`: pred eval rozvinie `...názov` spready v locale súboroch načítaním `export const názov = {…}` z importovaných doménových slovníkov (`apps/app/src/lawoss/i18n/*`); sekcia „Dynamic t()“ v `apps/app/src/lawoss/**` prijme šablónu, ktorej statický prefix zodpovedá aspoň jednému kľúču v `en.ts` (kľúče sú typované proti slovníkom), upstream zóna ostáva prísna | Od #87 locale súbory skladajú LAWOSS slovníky spreadom; naivný eval padal na `setupEn is not defined` a `ci-i18n` bol na `dev` červený | MČ | fix/i18n-audit-spready |
...
| `apps/desktop/electron-builder.yml` | `productName` → `LAWOSS`; publisher → `Omni-Legal-Products/lawoss`; icon files in `resources/icons/**` replaced by LAWOSS badge (binary); `artifactName` `legalwork-` → `lawoss-` (value-only); explicit `copyright:` naming Eigenwelt Labs and LAWOSS contributors; `LICENSE` and `NOTICE` added to `extraResources` | LAWOSS branding and fork-owned updater metadata; `appId` intentionally unchanged (keychain/user-data continuity — separate ADR if ever). Without an explicit `copyright:` electron-builder derives it from `author` and stamps a single holder onto a binary that is overwhelmingly upstream code; MIT requires the notice to travel with every copy, and a packaged `.dmg`/`.exe` is a copy | MČ | fix/updater-release-assets; plan/branding-pass-a-alfa; fix/licencna-atribucia |
...
| `apps/app/src/react-app/shell/app-root.tsx` | Route `/` → `<LawossLanding />` namiesto `<Navigate to="/prehlad" />` (+1 import, 1 riadok) | LAWOSS-lite začína na „Dnes“ (`/dnes`); v pro `landingPath("pro")` vracia `/prehlad`, správanie beze zmeny (test `lawoss-lite-shell`) | VŘ | feat/lawoss-lite |
| `apps/app/src/react-app/domains/session/sidebar/app-sidebar.tsx` | `useUiMode()` + `isSidebarItemVisible()` (+2 importy, +2 riadky); v lite sa nevykreslí „New task“, Tasks, Workflows, Recorder, Evals a sekcia Folders (`shown(…) &&` okolo existujúcich blokov) | Lite ukazuje len Dnes · Klienti a veci · Zeptat se (+ Poslední věci) z `LawossNav`; v pro `isSidebarItemVisible` vracia vždy `true` | VŘ | feat/lawoss-lite |
| `apps/app/src/react-app/domains/settings/shell/settings-page.tsx` | `getWorkspaceSettingsTabs()` a `getGlobalSettingsTabs()` prejdú `liteSettingsTabs(…, currentUiMode())` (za `hideCommercialTabs`); prázdna skupina Workspace v bočnom paneli `hidden`; prepínač workspace v hlavičke bočného panela sa v lite nevykreslí (`isWorkspaceSwitcherVisible(useUiMode())`) (+2 importy, 4 riadky) | Lite nastavenia: prehľad, AI, Personalizácia (kancelária), Vzhľad; v pro `liteSettingsTabs` vracia to isté pole; lite nesmie ukázať pojem „workspace“ | VŘ | feat/lawoss-lite |
| `apps/app/src/react-app/domains/settings/shell/settings-shell.tsx` | `useUiMode()` v `SettingsShell` (prekreslenie zoznamov záložiek) + `<UiModeSwitch />` vedľa `LanguageSwitcher` v oboch hlavičkách; kompaktný `WorkspaceMenu` sa v lite nevykreslí (`isWorkspaceSwitcherVisible`) (+3 importy, +3 riadky) | Prepnutie Jednoduchý/Pokročilý na jednom mieste, bez reštartu | VŘ | feat/lawoss-lite |
| `apps/app/src/react-app/domains/settings/pages/general-view.tsx` | Riadky prehľadu filtrované `isSettingsTabVisible(tab, currentUiMode())`; prázdna skupina sa nevykreslí; `useUiMode()` kvôli prekresleniu; v lite sa nevykreslí pätička `settings.tab_description_general` (+2 importy, +6 riadkov) | Prehľad nesmie v lite odkazovať na skryté záložky (rovnaký dôvod ako filter `HIDDEN_SETTINGS_TABS`); pätička hovorí o workspace, ktoré lite advokátovi neukazuje — v pro sa vykreslí ako doteraz | VŘ | feat/lawoss-lite |
| `apps/app/src/react-app/domains/session/chat/permission-approval-modal.tsx` | V `PermissionApprovalPanel` aj `PermissionApprovalModal` nad detailom príkazu `{lite && memoryWriteProposal ? <MemoryWriteNotice proposal={memoryWriteProposal} /> : null}`, kde `memoryWriteProposal = describeMemoryWrite(String(metadata.command ?? ""))` (+3 importy, +2× 1 riadok); pri karte v lite sa tlačidlo „Povoliť pre session“ nevykreslí (`lite && memoryWriteProposal ? null : …`, 2×); `readablePermissionLabel` v lite vráti namiesto „Bash“ a surového identifikátora oprávnenia text `lawoss.lite.permission_command` / `permission_other` (+`currentUiMode` import, 2 riadky) | Lite karta pri žiadosti o spustenie `okf-memory write …`: vec, záznam, dôvod, schvaľovateľ, náhľad/zápis po schválení, lehoty vždy potvrdzuje advokát; pravidlo „pre session“ by povolilo aj neskorší zápis s `--apply` bez karty, preto ho lite pri karte neponúka; pro nie je dotknuté vďaka podmienke `lite &&`, v pro sú tlačidlá a zvyšok panela beze zmeny | VŘ | feat/lawoss-lite |
| `apps/app/package.json` | +1 skript `test:lite-smoke` (`node scripts/lite-smoke.mjs`) | Smoke test LAWOSS-lite ↔ pro nad bežiacou appkou cez CDP a `window.__legalworkControl` (Task 8, `scripts/lite-smoke.mjs`, nový LAWOSS súbor) potrebuje spúšťací skript rovnako ako `test:voice-cdp` | VŘ | feat/lawoss-lite |
| `apps/app/src/react-app/shell/settings-route.tsx` | Za presmerovaním `/settings` → `/workspace/<id>/settings` +1 import a 3 riadky: v lite, ak je vybraná zložka vnútri registrovanej kancelárie, `<Navigate>` na nastavenia kancelárie (`liteSettingsWorkspace` v `lawoss/lite/office-scope.ts`) | Po konverzácii nad vecou je aktívna zložka veci; Personalizácia (profil kancelárie) by sa inak uložila do veci (`<vec>/Office/okf.config`) a rozbila zdieľanú pamäť kancelárie; pro a vložené nastavenia beze zmeny | VŘ | feat/lawoss-lite |
