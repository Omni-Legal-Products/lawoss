...
| `apps/desktop/electron/main.mjs` | `NAVIGATION_ALLOWLIST` pustí `file:` len v adresári bundlu appky (`fileDirectoryEntry`), nie ľubovoľný lokálny súbor; +`assertTrustedSender()` v `handleDesktopInvoke` a v `legalwork:shell:openExternal`; `openExternal` len http(s)/mailto; `file://` odkaz otvorí cez `shell.openPath` iba dokument (inak `showItemInFolder`) | Bezpečnostné spevnenie mosta okna (Electron checklist: validácia odosielateľa IPC, obmedzenie `openExternal` a navigácie); logika je v LAWOSS súbore `window-allowlist.mjs` | VŘ | fix/bezpecnostne-spevnenie |
| `apps/desktop/electron/browser-panel.mjs` | `window.open` z webového obsahu ide do `shell.openExternal` len pri http(s)/mailto (`isSafeExternalUrl`) | Webový obsah nemá dosiahnuť obslužné programy OS pre iné schémy | VŘ | fix/bezpecnostne-spevnenie |
| `apps/desktop/electron/ui-control-server.mjs` | Token porovnaný cez `timingSafeEqual`; discovery súbor s tokenom zapisovaný a dotiahnutý na `0600` | Súbor nesie bearer token, ktorým sa ovláda okno appky | VŘ | fix/bezpecnostne-spevnenie |
| `apps/server/src/server.ts`, `apps/server/src/utils.ts`, `apps/server/src/utils.test.ts` | +`tokensEqual()` (porovnanie v konštantnom čase cez SHA-256); `requireHost`/`requireHostToken` ho používajú namiesto `===` | Host token je vlastnícke oprávnenie; pri vzdialenom prístupe (0.0.0.0) nemá porovnanie prezrádzať časom | VŘ | fix/bezpecnostne-spevnenie |
| `.github/workflows/{alpha-macos-aarch64,alpha-windows-x64,ci-docx,ci-legalwork-ui-mcp,ci-tests,release-macos-aarch64}.yml` | Akcie tretích strán (`oven-sh/setup-bun`, `pnpm/action-setup`, `signpath/…`) pripnuté na commit SHA, tag v komentári | Workflowy majú prístup k podpisovým tajomstvám; posunutý tag by nemal zmeniť kód, ktorý ich vidí | VŘ | fix/bezpecnostne-spevnenie |
| `pnpm-workspace.yaml`, `apps/app/package.json`, `pnpm-lock.yaml` | `xlsx` 0.18.5 → 0.20.3 z oficiálnej distribúcie SheetJS (`cdn.sheetjs.com`, npm už novšie verzie nevydáva); `overrides` pre `@xmldom/xmldom` 0.9.x → ≥ 0.9.12 (cez `docxtemplater`) a `axios` → ≥ 1.16.0 (cez `@slack/web-api`) | Známe zraniteľnosti v knižniciach, ktoré spracúvajú súbory otvorené v appke (tabuľky, DOCX) a v HTTP klientovi; `pnpm audit --prod` high 67 → 44 | VŘ | fix/bezpecnostne-spevnenie |
