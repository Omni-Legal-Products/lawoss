...
| `apps/app/src/react-app/domains/session/artifacts/artifact-markdown-editor.tsx` | +`ViewModeProbe` (číta `viewMode$`) a ref `pristine` (pôvodné bajty + ich kanonická podoba z `initialNormalize`); `onChange` prechádza cez `restorePristineMarkdown()` z LAWOSS súboru `apps/app/src/lawoss/markdown/pristine.ts` | Undo až na nedotknutý dokument vracal kanonický Markdown (escapovanie `[`, značka odrážky, koncové LF) a draft ostal dirty (#90); mapuje sa len v rich-text režime, úmyselné zmeny v source režime ostávajú | VŘ | fix/markdown-undo-cisty-stav |
